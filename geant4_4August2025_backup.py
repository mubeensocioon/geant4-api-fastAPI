...
def create_visualization_image(image_path: str, viz_type: str, sim_request: SimulationRequest) -> VisualizationData:
    """Convert EPS to PNG and create base64 encoded image data"""
    viz_start_time = time.time()
    
    try:
        eps_path = f"{image_path}.eps"
        png_path = f"{image_path}.png"
        
        # Check if EPS file was generated
        if not os.path.exists(eps_path):
            return VisualizationData(
                description=f"Visualization file not generated: {eps_path}",
        ...
        if result.returncode != 0:
            ...
            return VisualizationData(
                description=f"Image conversion failed: {result.stderr}",
                generation_time=time.time() - viz_start_time
            )
        ...
        if os.path.exists(png_path):
            ...
            return VisualizationData(
                description="PNG file was not created successfully",
                generation_time=time.time() - viz_start_time
            )
            
    except subprocess.TimeoutExpired:
        return VisualizationData(
            description="Image conversion timed out",
            generation_time=time.time() - viz_start_time
        )
    except Exception as e:
    ...
    try:
        if sim_request.enable_visualization:
            ...
            try:
                ...
                logger.warning("ImageMagick not found - visualization will be limited")
        ...
        if sim_request.enable_visualization:
            # Create a mock EPS file for demonstration
            # In real implementation, this would be generated by Geant4
            eps_content = f"""%!PS-Adobe-3.0 EPSF-3.0
%%BoundingBox: 0 0 {sim_request.image_width} {sim_request.image_height}
%%Title: Geant4 Simulation Visualization
%%Creator: Geant4 Nuclear Physics API
%%EndComments

% Simple visualization placeholder
/{sim_request.background_color == "white" and "0" or "1"} setgray
0 0 {sim_request.image_width} {sim_request.image_height} rectfill

% Draw target nucleus (circle)
/Times-Roman findfont 12 scalefont setfont
{sim_request.background_color == "white" and "1 0 0" or "0 1 1"} setrgbcolor
{sim_request.image_width//2} {sim_request.image_height//2} 30 0 360 arc fill

% Draw beam line
{sim_request.background_color == "white" and "0 0 1" or "1 1 0"} setrgbcolor
3 setlinewidth
50 {sim_request.image_height//2} moveto
{sim_request.image_width//2 - 35} {sim_request.image_height//2} lineto stroke

% Add labels
{sim_request.background_color == "white" and "0" or "1"} setgray
({target}) show
50 {sim_request.image_height//2 + 20} moveto
({projectile} beam) show

% Add energy label
{sim_request.image_width//2} {sim_request.image_height - 30} moveto
({sim_request.beam_energy_mev} MeV) show

showpage
%%EOF"""
            
            eps_path = os.path.join(sim_dir, "simulation_viz.eps")
